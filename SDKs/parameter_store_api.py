...
def fetch_secrets(app):
    ...
    try:
        ...
        # access Reset Link
        parameter = ssm.get_parameter(Name="FRONTEND_RESET_URL", WithDecryption=True)
        ...
        parameter = ssm.get_parameter(Name="SES_FROM_EMAIL", WithDecryption=True)
        SES_FROM_EMAIL = parameter["Parameter"]["Value"]
        app.config["SES_FROM_EMAIL"] = SES_FROM_EMAIL

        # access ADMIN email - send request for beta test
        parameter = ssm.get_parameter(Name="ADMIN_EMAIL", WithDecryption=True)
        ADMIN_EMAIL = parameter["Parameter"]["Value"]
        app.config["ADMIN_EMAIL"] = ADMIN_EMAIL

        # BASE secure address for Sling It application
        parameter = ssm.get_parameter(Name="BASE_URL", WithDecryption=True)
        ...
        logger.error("SSM error response: %s", resp)
