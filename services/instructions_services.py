...
class InstructionServices:
    ...
    def fetch_user_instructions(user_id):
        ...
        try:
            ...
            raise (
                f"Error in InstructionServices -> fetch_user_instructions: {e}"
            ) from e
    ...
    def process_book_instruction(request, book_id):
        ...
        try:
            ...
            db.session.rollback()
            raise type(e)(
                f"Error in InstructionServices - process_book_instruction"
            ) from e
    ...
    def create_instruction_association(book_id, instruction_id):
        ...
        try:
            ...
            db.session.commit()
            return {
                "message": f"Successful association of instruction {instruction_id} to book {book_id}!"
            }
        except Exception as e:
            db.session.rollback()
            raise type(e)(
    ...
    def build_instructions(instances):
        ...
        try:
            return [
                Instruction.serialize(instruction_instance)
                for instruction_instance in instances
            ]
        except Exception as e:
            raise type(e)(f"InstructionServices - build_instructions error: {e}") from e
