...
class IngredientServices:
    ...
    def create_option_association(component, book_id, option_id):
        ...
        try:
            ...
            db.session.commit()
            return {
                "message": f"Successful association of option {option_id} to book {book_id}!"
            }
        except Exception as e:
            db.session.rollback()
            raise type(e)(
    ...
    def process_ingredient_components(book_id, ingredients):
        ...
        for ingredient in ingredients:
            ...
            try:
                ...
                amount = AmountServices.process_amount(amount=amount, book_id=book_id)
                unit = UnitServices.process_unit(unit=unit, book_id=book_id)

                ingredients_data.append({"item": item, "amount": amount, "unit": unit})

            except Exception as e:
                raise type(e)(
                    f"IngredientsRepo -> process_ingredient_components error: {e}"
                ) from e
    ...
    def build_ingredients(instances):
        ...
        try:
            for ingredient in instances:
                amount = (
                    QuantityAmount.serialize(ingredient.amount)
                    if ingredient.amount
                    else None
                )
                unit = (
                    QuantityUnit.serialize(ingredient.unit) if ingredient.unit else None
                )
            ...
            raise type(e)(f"Ingredient_services - build_ingredients error: {e}") from e
