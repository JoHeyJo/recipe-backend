    ...
    def signup(user_name, first_name, last_name, email, password):
        ...
        try:
            ...
            if "users_user_name_key" in str(e.orig):
                ...
                raise EmailAlreadyRegisteredError("This email is already taken.") from e
            else:
...
class QuantityAmountRepo:
    ...
    def query_book_amounts(book_id: int) -> list[QuantityAmount]:
        ...
        try:
            ...
            raise type(e)(f"QuantityAmountRepo - query_book_amounts error: {e}") from e
    ...
    def query_user_amounts(user_id):
        ...
        try:
            amounts = UserRepo._query_by_user(
                QuantityAmount, AmountBook, AmountBook.amount_id
            )(user_id)
...
class QuantityUnitRepo:
    ...
    def query_user_units(user_id):
        """Return user's units"""
        try:
            units = UserRepo._query_by_user(QuantityUnit, UnitBook, UnitBook.unit_id)(
                user_id
            )

            return [QuantityUnit.serialize(unit) for unit in units]
        except Exception as e:
            raise type(e)(f"QuantityUnitRepo - query_user_units error: {e}") from e
    ...
    def query_book_units(book_id: int) -> list[QuantityAmount]:
        ...
        try:
            ...
            raise type(e)(f"QuantityUnitRepo - query_book_units error: {e}") from e
...
class ItemRepo:
    ...
    def query_user_items(user_id):
        """Return user's items"""
        try:
            items = UserRepo._query_by_user(Item, ItemBook, ItemBook.item_id)(user_id)
            return [Item.serialize(item) for item in items]
        except Exception as e:
...
class IngredientsRepo:
    ...
    def query_ingredient(primary_key):
        ...
        try:
            ...
            raise type(e)(f"Error in IngredientsRepo -> query_ingredient: {e}") from e
    ...
    def query_ingredients(primary_keys: list[int]) -> list[Ingredient]:
        ...
        try:
            ...
            raise type(e)(f"IngredientsRepo - query_ingredients error: {e}") from e
...
class BookRepo:
    ...
    def query_user_book_by_pk(book_pk):
        ...
        try:
            ...
            raise type(e)(f"BookRepo - query_user_book_by_pk error: {e}") from e
    ...
    def query_user_books(user_id):
        ...
        try:
            ...
            return [
                {**Book.serialize(user_book.book), "book_role": user_book.role.value}
    ...
    def query_user_instructions(user_id):
        ...
        try:
            instructions = UserRepo._query_by_user(
                Instruction, BookInstruction, BookInstruction.instruction_id
            )(user_id)
            return [Instruction.serialize(instruction) for instruction in instructions]
        except Exception as e:
...
class RecipeIngredientRepo:
    ...
    def create_ingredient(recipe_id, item_id, quantity_unit_id, quantity_amount_id):
        ...
        try:
            ...
            raise type(e)(f"RecipeIngredientRepo - create_ingredient error:{e}") from e
...
class RecipeBookRepo:
    ...
    def query_recipe_book(book_id, recipe_id):
        ...
        try:
            ...
            raise type(e)(f"RecipeBookRep - query_recipe_book error:{e}") from e
    ...
    def does_recipe_exist_in_shared_inbox(shared_book_id, shared_recipe_id):
        ...
        try:
            ...
            raise type(e)(
                f"RecipeBookRep - does_recipe_exist_in_shared_inbox error:{e}"
            ) from e
...
class UserBookRepo:
    ...
    def query_user_book_ids(user_id, book_id):
        ...
        try:
            stmt = db.select(UserBook.book_id).where(UserBook.user_id == user_id)
            return db.session.execute(stmt).scalars().all()
        except Exception as e:
            raise type(e)(f"UserBookRepo - query_user_book_ids error:{e}") from e
...
class BookInstructionRepo:
    ...
    def query_book_instruction(book_id, instruction_id):
        ...
        try:
            return db.session.get(
                BookInstruction, {"book_id": book_id, "instruction_id": instruction_id}
            )
        except Exception as e:
            raise type(e)(
                f"BookInstructionRepo - query_book_instruction error :{e}"
            ) from e
...
class RecipeInstructionRepo:
    ...
    def query_recipe_instruction(recipe_id, instruction_id):
        ...
        try:
            ...
            raise type(e)(
                f"RecipeInstructionRepo - query_recipe_instruction error :{e}"
            ) from e
